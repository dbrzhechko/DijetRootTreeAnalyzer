...
def main(options,args):
    ...
    for box,lumi in zip(boxes,lumiFloat):
        ...
        if options.theorySys:
            theoryString  ='--theorySys %f'%options.theorySys

        for massPoint in massIterable(options.mass):
            exec_me('python python/WriteDataCard.py -m %s --mass %s -i %s -l %f -c %s -b %s -d %s %s %s %s %s %s %s %s %s'%(model, massPoint, options.inputFitFile,1000*lumi,options.config,box,options.outDir,signalDsName,backgroundDsName[box],penaltyString,signalSys,xsecString,decoString,multiString,theoryString),options.dryRun)    
            if options.bayes:
...
if __name__ == '__main__':
    ...
    parser.add_option('--theorySys',dest="theorySys", default=1.06,type="float",
